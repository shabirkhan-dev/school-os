---
title: Production Roadmap
description: Phased NestJS backend spine and product build plan.
---
> **Purpose:** This README defines how we will build School OS from the clean `apps/nest-api` NestJS app onward. It summarizes the full idea, the product philosophy, the backend/frontend/AI architecture, and the exact production roadmap. It is written so we can share it with other AI tools, collaborators, reviewers, and future funders before and during development.

---

## 1. Project Vision

School OS is **not** another school ERP, digital register, or dashboard.

School OS is the **trust engine for Pakistani school networks** — mobile-first, WhatsApp-native,
and designed with **Aga Khan Schools (AKES)**-scale multi-campus operators in mind (Karachi,
Lahore, Islamabad). The first product milestone is the ninety-second demo:

```txt
QR scan at gate → attendance recorded → WhatsApp parent alert → live principal dashboard (<5s)
```

Traditional school software usually does this:

```txt
User enters data
System stores data
User checks dashboard manually
User takes action manually
```

School OS should do this:

```txt
Student arrives → attendance event → parent alert on WhatsApp
→ live dashboard updates → optional AI risk signal (with human approval)
→ audit log records everything
```

Workflow, policy, and AI engines support that loop — they are not the headline. Ship the
scan→alert→dashboard hook before expanding scope.

The long-term platform direction may grow into a broader **Community OS**, but the build focus now
is strict:

```txt
Start with School OS trust loop.
Build slowly.
One production-quality vertical slice at a time.
```

---

## 2. Current Starting Point

The monorepo already exists and uses:

```txt
Bun workspaces
Turborepo
Next.js web app
Expo mobile app
NestJS API app
Docs app
Shared packages
CI/CD, Docker, security workflows, dev container, hooks
```

The backend production spine starts now from:

```txt
apps/nest-api
```

We intentionally start the NestJS backend clean and add concepts step by step.

---

## 3. Technical Decisions

### Runtime and Tooling

```txt
Package manager: Bun
Workspace orchestration: Turborepo
Main backend framework: NestJS
Production backend runtime: Node-compatible Nest build first
Database: PostgreSQL
ORM/query layer: Drizzle ORM
Validation: Zod
API style first: REST
Frontend: Next.js
Mobile: Expo / React Native
Docs: docs app / Markdown
```

Bun stays important for fast local development and monorepo commands. NestJS remains the main backend framework because School OS needs structured modules, dependency injection, guards, interceptors, queues, WebSockets, auth, testing, and production architecture.

Drizzle is chosen because it is TypeScript-first, lightweight, SQL-friendly, and a good fit for PostgreSQL.

---

## 4. Core Product Philosophy

Everything we build should follow this principle:

```txt
Do not make teachers, admins, parents, or principals do more data entry.
Use software to reduce work, guide decisions, and execute safe workflows.
```

School OS must be:

```txt
AI-native
workflow-native
policy-native
task-first
privacy-first
offline-aware
local-language ready
audit-heavy
teacher-first
student-safe
parent-connected
integration-ready
```

---

## 5. What Makes School OS Different

A normal school ERP stores records:

```txt
students
teachers
attendance
fees
exams
reports
```

School OS adds operating intelligence:

```txt
policies
workflows
tasks
approvals
AI assistance
risk detection
parent communication
audit logs
notifications
integrations
offline workflows
```

One-line positioning:

```txt
School OS is the trust engine for schools — scan, WhatsApp alert, live dashboard, then grow.
```

Product differentiation (what parents and principals feel):

```txt
Parents feel it in seconds — WhatsApp arrival alerts, not another portal login
Safe by architecture — tenant scope, guardian consent, audit logs, AI gated by policy
Grows without replatforming — attendance first, then communication, finance, and AI on one spine
```

---

## 6. Main Engines

These engines are the heart of School OS. Domain modules like Students, Attendance, Fees, Homework, and Cleanliness plug into these engines.

### 6.1 Workflow Engine

Manages multi-step school processes.

Examples:

```txt
admission workflow
attendance follow-up workflow
homework review workflow
teacher leave workflow
cleanliness task workflow
student risk intervention workflow
fee reminder workflow
```

A workflow has:

```txt
definition
run
steps
owner
status
deadline
approval
escalation
audit events
```

### 6.2 Policy Engine

Turns school rules into executable logic.

Examples:

```txt
If student absent 3 days → create teacher follow-up task
If attendance below 80% → notify parent and principal
If cleaning issue unresolved 24h → escalate
If required admission document missing → block final approval
```

### 6.3 Task Engine

Every unresolved issue becomes a task.

Examples:

```txt
Call parent
Verify document
Review homework
Approve admission
Check classroom cleanliness
Follow up absent student
Assign substitute teacher
```

### 6.4 Notification Engine

Handles communication through:

```txt
WhatsApp (primary parent channel from Product Milestone 1)
in-app and push (teachers, staff)
email and SMS (fallback)
```

Guardian opt-in, utility templates, quiet hours, and delivery audit logs are required before
production pilots. See [Product Vision](/docs/product-vision) for compliance notes.

### 6.5 AI Orchestration Engine

AI must assist, not control the system directly.

Safe AI flow:

```txt
AI reads allowed context
AI proposes action
Policy engine checks action
Permission engine checks action
Human approval happens if needed
Command handler executes action
Audit log records everything
```

AI should never directly update critical records without going through backend commands, policy checks, and audit logs.

### 6.6 Document Intelligence Engine

Used for admissions and admin work:

```txt
OCR
field extraction
document classification
missing document detection
form prefill
human verification
```

### 6.7 Integration Engine

For future external systems:

```txt
SMS provider
WhatsApp provider
email provider
payment gateway
object storage
biometric/RFID optional
camera AI optional
government export formats
calendar integrations
```

### 6.8 Audit and Governance Engine

Every important action must be traceable:

```txt
who did it
when
which organization/school
what changed
why
from which IP/device/request
whether AI was involved
whether approval was given
```

---

## 7. Product Modules

### Core Records

```txt
Organizations
Schools
Users
Roles
Permissions
Students
Guardians
Teachers
Academic Years
Classes
Sections
Enrollments
```

### School Operations

```txt
Attendance
Homework
Assessments
Timetable
Cleanliness
Facilities
Inventory
Transport later
Fees later
```

### Intelligence

```txt
Student risk detection
Attendance pattern detection
Homework weakness detection
Teacher workload insight
Principal cockpit
AI summaries
AI message drafts
```

### Communication

```txt
Parent updates
Announcements
Absence alerts
Meeting requests
Homework summaries
Fee reminders later
```

### Governance

```txt
Audit logs
Consent records
AI usage logs
Approvals
Data retention
Tenant isolation
Security policies
```

---

## 8. Backend Architecture Direction

The backend starts in:

```txt
apps/nest-api
```

Initial structure:

```txt
apps/nest-api/src/
  main.ts
  app.module.ts
  app.setup.ts

  common/
    decorators/
    filters/
    guards/
    interceptors/
    middleware/
    pipes/
    types/
    utils/

  config/
    env.schema.ts
    app.config.ts
    database.config.ts
    config.module.ts
    app-config.service.ts

  database/
    database.module.ts
    database.provider.ts
    database.tokens.ts
    database.types.ts
    schema/
      index.ts

  modules/
    health/
    organizations/
    schools/
    users/
    auth/
```

Later structure:

```txt
modules/
  academic-structure/
  students/
  guardians/
  teachers/
  attendance/
  tasks/
  policies/
  workflows/
  notifications/
  audit-logs/
  ai/
  documents/
  integrations/
  realtime/
```

---

## 9. Backend Rules

### Controller / Service / Repository

```txt
Controller = HTTP layer only
Service = business and use-case logic
Repository = database queries only
Schema/DTO = input validation and type shape
Guard = authentication/authorization
Interceptor = response/logging/metrics wrapping
Filter = error formatting
Module = feature boundary
```

Do not put business logic in controllers.

Do not put HTTP exceptions inside repositories unless there is a very strong reason.

Do not let AI or frontend directly mutate database records.

---

## 10. Import and Naming Rules

Use clear aliases later:

```txt
@common/*
@config/*
@database/*
@modules/*
@shared/*
```

File naming:

```txt
kebab-case files
PascalCase classes
camelCase variables/functions
is/has/can/should for booleans
```

Examples:

```txt
create-school.schema.ts
schools.controller.ts
schools.service.ts
schools.repository.ts
current-user.decorator.ts
```

---

## 11. API Response Standard

Success response:

```json
{
  "success": true,
  "statusCode": 200,
  "requestId": "req_123",
  "timestamp": "2026-07-01T00:00:00.000Z",
  "data": {}
}
```

Error response:

```json
{
  "success": false,
  "statusCode": 400,
  "code": "VALIDATION_ERROR",
  "message": "Request validation failed",
  "requestId": "req_123",
  "timestamp": "2026-07-01T00:00:00.000Z",
  "path": "/api/v1/schools",
  "method": "POST",
  "errors": []
}
```

---

## 12. Multi-Tenant Rule

Every school belongs to an organization.

Most future data must be tenant-safe:

```txt
organizationId
schoolId when relevant
actorUserId when relevant
```

Repository methods should accept object params:

```ts
findById(params: {
  organizationId: string;
  schoolId: string;
})
```

Never query tenant-owned data without tenant scope.

Cross-tenant access should usually return `404`, not `403`, to avoid revealing that another tenant’s resource exists.

---

## 13. AI Safety Rule

AI can:

```txt
summarize
extract
draft
suggest
classify
flag
prepare
```

AI cannot directly:

```txt
create final student records
change grades
send sensitive messages
approve welfare/discipline actions
delete records
access all tenant data freely
```

AI actions must go through:

```txt
permission check
policy check
approval if required
command handler
audit log
```

---

## 14. Language and Localization

School OS should be multilingual from the design level.

Start with:

```txt
English
Urdu
```

Later possible:

```txt
Khowar
Shina
Burushaski
Wakhi
```

Do not hardcode user-facing text deeply into business logic. Prefer error codes and message keys where possible.

Example:

```json
{
  "code": "STUDENT_NAME_REQUIRED",
  "messageKey": "students.errors.nameRequired"
}
```

---

## 15. Product Milestone 1 — Trust Engine Demo

This is the **product** definition of done — distinct from backend engineering phases below.
Engineering Phases 0–6 build the spine; **the demo ships when this loop works end-to-end.**

Target scenario (AKES Karachi reference):

```txt
Hassan Raza · Class 7-B · Gate A scanned at 8:17 AM
→ WhatsApp to guardian Amina Khan: "Hassan arrived safely at 8:17 AM"
→ Principal dashboard: 247 in · 3 absent · 1 late
```

Build:

```txt
tenants, campuses, classes, sections, students, guardians
QR token generation and gate scanner (mobile + web)
attendance mark + idempotent event emission
WhatsApp outbox worker (Meta Cloud API or approved BSP)
principal live dashboard (web)
parent attendance feed (mobile optional; WhatsApp is the hook)
guardian consent records and delivery audit log
```

Definition of done:

```txt
scan → record → WhatsApp → dashboard in under 5 seconds (p95, single campus pilot)
guardian opt-in enforced before first message
no cross-tenant data leakage
attendance and notification attempts audited
```

Aligns with [Product Vision](/docs/product-vision) Phase 1 and
[Product System Design](/docs/product-system-design) Phase 1: Smart Attendance MVP.

---

## 16. Roadmap Overview — Backend Engineering Phases

### Phase 0 — Clean Nest Foundation

Goal: make the Nest API clean, predictable, and production-shaped.

Build:

```txt
health endpoint
global prefix /api/v1
URI versioning
config module
env validation
request ID middleware
response interceptor
exception filter
Zod validation pipe
basic app setup
```

Definition of done:

```txt
GET /api/v1/health works
consistent response shape
consistent error shape
invalid input returns validation error
app starts cleanly with Bun workspace command
```

---

### Phase 1 — Database Foundation

Goal: connect PostgreSQL with Drizzle.

Build:

```txt
Docker Postgres or hosted Postgres
Drizzle install
DatabaseModule
schema folder
database provider
migration config
organizations table
schools table
users table basic
```

Definition of done:

```txt
migration runs
API can read/write organizations and schools
DB connection closes cleanly on shutdown
```

---

### Phase 2 — Organization and School Slice

Goal: first real tenant slice.

Build:

```txt
OrganizationsModule
SchoolsModule
create organization
create school
list schools
get school
update school
soft delete school later
```

Definition of done:

```txt
POST organization works
POST school under organization works
school code unique per organization
tenant-safe reads
unit tests for service
basic e2e test
```

---

### Phase 3 — Auth and Users

Goal: secure the system.

Build:

```txt
UsersModule
AuthModule
password hashing
login
JWT access token
refresh tokens
current user decorator
current organization decorator
JWT guard
```

Definition of done:

```txt
user can login
protected route requires token
invalid password fails safely
refresh token stored hashed
logout revokes session
```

---

### Phase 4 — Roles and Permissions

Goal: role-based access control.

Build:

```txt
roles
permissions
user roles
role permissions
permissions guard
@Auth decorator
seed default permissions
```

Definition of done:

```txt
admin can create school
teacher cannot create school
permission denied returns 403
platform admin rules are explicit
```

---

### Phase 5 — Academic Structure

Goal: define school academic setup.

Build:

```txt
academic years
classes
sections
teachers
teacher assignments
```

Definition of done:

```txt
school can create academic year
school can create class and section
teacher can be assigned to section
all reads tenant-safe
```

---

### Phase 6 — Students and Enrollment

Goal: manage student records properly.

Build:

```txt
students
guardians basic later
student enrollments
student status
admission number uniqueness per school
```

Definition of done:

```txt
create student
update student
list students
enroll student into section and academic year
cannot enroll into another tenant’s school
```

---

### Phase 7 — Attendance

Goal: first daily school workflow.

Build:

```txt
mark attendance
read section attendance
read student attendance history
attendance status counts
teacher assignment access check
emit attendance event for notification outbox (WhatsApp parent alert)
```

Definition of done:

```txt
teacher/admin can mark attendance
unassigned teacher cannot mark attendance
invalid student IDs rejected
attendance creates audit log
attendance event emitted
```

---

### Phase 8 — Audit Logs

Goal: trust and traceability.

Build:

```txt
audit log table
audit log service
audit events for important actions
audit log listing with filters
```

Definition of done:

```txt
student creation audited
attendance marking audited
role changes audited
sensitive values not stored in audit metadata
```

---

### Phase 9 — Tasks, Policies, and Workflows v1

Goal: make School OS more than CRUD.

Build:

```txt
tasks table
policy definitions simple
workflow runs simple
absence follow-up policy
cleaning issue task later
```

First workflow:

```txt
Student absent 3 consecutive days
→ create teacher follow-up task
→ notify teacher/principal later
→ audit log
```

Definition of done:

```txt
policy can trigger task
task has owner/status/deadline
workflow is traceable
```

---

### Phase 10 — Notifications and Queue

Goal: background work.

Build:

```txt
Redis
BullMQ
notification queue and outbox worker
WhatsApp adapter (primary) + email/SMS placeholders
parent arrival/absence utility templates with guardian consent check
```

Definition of done:

```txt
job added to queue
worker processes job
failed jobs retry
worker logs job lifecycle
```

---

### Phase 11 — AI Orchestration v1

Goal: safe AI layer.

Start with one feature:

```txt
Admission document assistant
```

Flow:

```txt
upload document
extract fields
prefill admission draft
admin verifies
policy checks required fields
approved command creates student
```

Definition of done:

```txt
AI output never directly writes final record
AI request logged
AI output validated
human approval required
```

---

### Phase 12 — Web Admin v1

Goal: usable web interface.

Build:

```txt
login
organization/school setup
student list
student create form
academic setup
attendance screen
basic dashboard
```

UI principle:

```txt
task-first, not menu-first
```

---

### Phase 13 — Teacher Mobile v1

Goal: daily teacher workflow.

Build:

```txt
login
my sections
mark attendance
offline draft later
student list
follow-up tasks
```

---

### Phase 14 — Production Readiness

Build:

```txt
Swagger/OpenAPI
unit tests
e2e tests
Docker
CI
security hardening
rate limiting
structured logging
metrics
health checks
```

---

## 17. First Build Target Now

We start with **Phase 0 only**.

Do not build database yet if app setup is not clean.

First target:

```txt
GET /api/v1/health
```

Then add:

```txt
app.setup.ts
config module
env validation
request ID middleware
response interceptor
exception filter
Zod pipe
```

---

## 18. Development Commands

From repo root:

```bash
bun install
bun run preflight
bun --cwd apps/nest-api run start:dev
```

If the app is clean:

```bash
curl http://localhost:3000/api/v1/health
```

Expected response:

```json
{
  "success": true,
  "statusCode": 200,
  "requestId": "...",
  "timestamp": "...",
  "data": {
    "status": "ok",
    "service": "school-os-api"
  }
}
```

---

## 19. Definition of Done for Every Slice

A feature is not done until:

```txt
code works
validation exists
tenant safety considered
errors are clean
audit need checked
tests exist where needed
API shape is documented
no secrets in logs
no random architecture shortcuts
```

---

## 20. What Not To Do Yet

Do not start with:

```txt
AI agents
camera attendance
homework checker
Community OS
GraphQL
microservices
mobile offline sync
payment system
full dashboards
```

Those come later.

Start with the backend spine.

---

## 21. Other AI Review Prompt

Use this prompt when asking another AI to review the plan:

```txt
We are building School OS, the trust engine for Pakistani school networks (AKES-scale reference).
The first product milestone is Smart Attendance: QR scan → WhatsApp parent alert → live principal
dashboard in under five seconds. The backend starts with a clean NestJS app inside a Bun +
Turborepo monorepo. We use PostgreSQL, Drizzle ORM, Zod validation, REST first, and
production-grade architecture. Workflow, policy, and AI engines support the trust loop — they are
not the headline for Month 1–3.

Please review this README as a senior backend/product architect. Focus on:
1. Whether Product Milestone 1 (attendance + WhatsApp) is achievable on the backend spine order.
2. Whether engineering phases are in the correct order relative to the trust-engine demo.
3. Whether WhatsApp, consent, and audit are placed early enough for Pakistani school pilots.
4. What should be removed from the first 3 months.
5. What hidden risks exist in multi-tenant school software handling minors.
6. How to make this more fundable without overpromising ERP breadth.

Do not suggest generic ERP features unless they support the scan→alert→dashboard hook.
```

---

## 22. References and Technical Anchors

- NestJS documentation: https://docs.nestjs.com
- NestJS CLI/workspaces: https://docs.nestjs.com/cli/monorepo
- Bun workspaces: https://bun.com/docs/pm/workspaces
- Turborepo docs: https://turborepo.com/docs
- Drizzle PostgreSQL docs: https://orm.drizzle.team/docs/get-started/postgresql-new
- Drizzle migrations: https://orm.drizzle.team/docs/migrations
- PostgreSQL: https://www.postgresql.org/docs/
- OWASP LLM / GenAI security: https://owasp.org/www-project-top-10-for-large-language-model-applications/
- UNESCO AI in education: https://www.unesco.org/en/digital-education/artificial-intelligence

---

## 23. Final Build Principle

```txt
Slow is smooth.
Smooth is fast.
```

We will not rush into features.

We will build the spine first, then add intelligence layer by layer.

The first practical milestone is:

```txt
Clean Nest API foundation with /api/v1/health and production response/error structure.
```
