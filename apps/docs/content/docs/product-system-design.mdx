...
- Portfolio spec notes (School Management OS — trust engine positioning)
- GitHub repo README and product vision docs

Repo inputs:

- `README.md`, `PROJECT.md`, `AGENTS.md`, `DESIGN.md`
- `/docs/product-vision`, `/docs/production-roadmap`
...
```text
Student arrives -> attendance event -> WhatsApp parent alert -> live dashboard -> AI risk/update signal
```

First production pilots target **Pakistani private and network schools** (AKES as design
reference): ~200–2K students per campus, multi-campus chains, mid-range Android devices, and
97%+ WhatsApp open rates for guardian alerts.
...
- NestJS API as the primary product API.
...
```text
                    ...
                    +----------+-----------+
                               |
+----------------+     +-------v--------+      +------------------+
| apps/web       |     | apps/nest-api  |      | apps/mobile      |
...
| Product API | `apps/nest-api` | Auth, tenancy, domain APIs, webhooks | Primary backend for production |
| Worker | `apps/nest-api/src/workers` | Outbox, reminders, notification dispatch, AI jobs | Same codebase at first to avoid a premature service split |
...
Until then, keep them as modules inside `apps/nest-api/src/modules`.
...
- Browser/mobile -> Nest API.
- Nest API -> payment provider, WhatsApp provider, map provider, object storage.
- Nest API -> FastAPI AI service later, only through a typed internal client.
...
- Public contracts use OpenAPI (Swagger). TypeScript clients use typed fetch or generated SDKs.
...
- Pakistani school networks are typically small to mid-size (200–2K students per campus).
...
Expose versioned product APIs under `/api/v1`. Auth routes live under `/api/v1/auth` in Nest;
new product routes should follow the same versioned prefix.
...
server-side authorization and minimal DTOs. For School OS, use direct Nest API calls through a
...
The current web auth context uses Nest cookie sessions; mobile uses SecureStore-backed tokens.
Before production, standardize on cookie/BFF style auth for the web and SecureStore-backed
sessions for mobile.
...
```text
apps/nest-api/src/modules/attendance
...
- Pick one production API: NestJS (`apps/nest-api`).
...
| API framework | NestJS for primary product API |
...
See [Multi-Tenancy](/docs/multi-tenancy) for roles, permissions, policies, and request context in
detail.
...
2. WhatsApp provider: Meta WhatsApp Cloud API directly, Twilio, or a **Pakistan-routing regional BSP**
   (evaluate Infobip and local Meta partners for PK delivery and template approval).
3. Payment provider priority by market: **Razorpay/Stripe for international**, plus local PSPs
   (**Paymob, JazzCash, Easypaisa**) for fee reminders in Phase 4.
...
6. Data residency target for first market: **Pakistan (Asia/Karachi)** for p95 latency and
...
- NestJS Helmet and security middleware:
